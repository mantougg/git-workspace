...
// F-35：Processes 表格分页（默认 5 条/页，最新在前）
// 传全量数据给 n-data-table，由组件内部切片分页；reactive + onChange
// 回调保持外部状态与 naive-ui 内部状态双向同步。
...
const processPagination = reactive({
  ...
  showSizePicker: true,
  pageSizes: [5, 10, 20],
  onChange: (page: number) => { processPagination.page = page; },
  onUpdatePageSize: (pageSize: number) => { processPagination.pageSize = pageSize; processPagination.page = 1; },
});
