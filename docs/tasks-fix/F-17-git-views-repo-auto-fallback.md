...
- [x] 新增 `src/composables/useCurrentRepo.ts`：解析顺序
  `route.query.repo` → `repoStore.currentRepoPath` → 当前工作区仓库列表
  第一个（store 列表为空时先 `loadRepositories` 拉取），解析成功回写
  store；工作区无仓库才返回空
- [x] 六个 Git 视图（GitGraph / BranchManager / StashManager /
  WorktreeManager / Reflog / DiffViewer）的 `onMounted` 改用该 composable
- [x] 工作区确实无仓库时保持原有提示 + 回跳变更页行为

## 验收标准

- [x] 应用重启后未勾选任何仓库，直接从 SideNav 进提交图/分支/Stash/
  Worktree/Reflog，自动展示当前工作区第一个仓库的数据，无警告
- [x] 变更页勾选仓库后再进 Git 视图，展示的仍是勾选的仓库（既有语义不变）
- [x] 带 `?repo=` query 跳转的行为不变（优先级最高）
- [x] 工作区没有任何仓库时仍给出提示（不崩）
...
- 当前状态：✅ 已完成
- 最近更新：2026-09-02 验收完成，工作区仓库回退链与六个 Git 视图接入已验证
...
| 2026-08-28 | 🟦 | 开始修复 |
| 2026-09-02 | ✅ | 验收完成：六个 Git 视图已接入工作区仓库回退链，验证 pnpm build 通过 |
