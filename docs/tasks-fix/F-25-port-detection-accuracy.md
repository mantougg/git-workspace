...
- [x] 收敛 runtime_check_port 与启动前预检的端口判定口径
- [x] 覆盖更完整的监听形态与占用场景，避免只测 loopback
- [x] 保持占用方 PID / 进程名 / 可执行路径的可行动提示
- [x] 补充端口空闲 / 占用 / 误判回归测试

## 验收标准

- [x] 端口被占用时不会再误报空闲
- [x] runtime_check_port 与 preflight 的结论一致
- [x] 端口诊断仍能给出占用方信息
- [x] 相关测试通过
...
- 当前状态：✅ 已完成
- 最近更新：2026-09-02 修复完成，统一探测口径与占用方解析已落地
...
| 2026-09-02 | 🟦 | 开始修复：确认 loopback bind 无法覆盖 wildcard/IPv6 监听，准备复用统一端口探测 |
| 2026-09-02 | ✅ | 修复完成：新增共享 TCP 端口探测，覆盖 IPv4 wildcard、IPv6 与 loopback；统一 Port Manager/启动预检；修正 netstat 精确端口匹配和 lsof 表头解析。验证：端口相关测试通过、局部 rustfmt --check 通过；全量 port 过滤测试仅剩既有 Runtime 环境失败 |
