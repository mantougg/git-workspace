...
- [ ] 两个 `n-spin` 增加专用 class，样式上参与 `.main-body` 的 flex
  布局（左侧固定 280px、右侧 `flex: 1; min-width: 0`，高度 100%，
  `.n-spin-content` 高度链打通）
- [ ] `.set-list` / `.set-detail` 高度 100%
- [ ] 空状态（左侧列表 `n-empty`、右侧详情 `n-empty`）在各自区域
  水平垂直居中

## 验收标准

- [ ] 无 Change Set 时，左侧空状态在列表区居中展示（含新建按钮），
  右侧详情区空状态同样居中且区域占满
- [ ] 有数据时列表/详情布局与之前一致，不出现双向滚动条或塌陷
- [ ] loading 时 spin 遮罩覆盖各自区域
