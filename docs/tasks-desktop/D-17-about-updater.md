...
- [ ] 「关于」页（路由 `/about`，设置分组）：应用图标、名称、版本号、GitHub 仓库地址、开源协议（MIT）
- [ ] 应用内更新状态机：检查 → 新版本提示（版本号 + 更新日志）→ 下载（进度条）→ 安装 → 重启生效
- [ ] 失败兜底：error 提示 +「前往 GitHub Releases 手动下载」链接（覆盖 Linux deb/rpm 等无法应用内更新的场景）
- [ ] 数据源遵循 F-07 延伸约定：协议 / 仓库地址经 package.json 字段构建期注入，组件内零硬编码
...
- [ ] SideNav「设置」分组出现「关于」入口，图标/高亮行为与其他条目一致
- [ ] 关于页正确展示：图标、名称、`v<package.json version> by <author>`、GitHub 链接（浏览器打开）、MIT License
- [ ] 「检查更新」各状态正确：检查中禁用 → 无更新提示 → 有更新显示版本号/日志/下载按钮 → 进度条 → 提示重启 → `restart_app` 重启
- [ ] 检查/下载失败时出现手动下载兜底链接
- [ ] `pnpm build`（含 vue-tsc）与 `cargo check` 通过
- [ ] 版本号/作者/协议/仓库地址无组件内硬编码（符合 F-07 及 §5.4 数据源表）
