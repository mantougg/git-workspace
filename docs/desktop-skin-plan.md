...
> 8. **自定义无边框标题栏**：一期/二期明确不做，三期以后随整体成熟度重新评估（非永久排除）。
...
- **自定义无边框标题栏：一期/二期不做**（自绘窗口控制按钮/拖拽区/Windows snap 适配成本高），三期以后随整体成熟度重新评估——它是最后残留的「套壳痕迹」，但不值得现在付出成本。
...
### 三期：Git Client Experience（可选，独立评估）

1. 变更视图三栏联动：仓库树 + 提交图 + diff（参照 Fork/IDEA Git 工具窗口）。
2. 面板 splitter 位置记忆（localStorage 按视图 key）。
3. 重新评估自定义无边框标题栏（此时它是最后残留的套壳痕迹，成本/收益再权衡）。
