...
- [ ] 目标结构（§4.7）：`watch/{mod.rs, debounce.rs, classify.rs, impact.rs, submit.rs, tests.rs}`
- [ ] `debounce.rs`：notify 事件收集和去抖
- [ ] `classify.rs`：路径分类和 `ignore_path`
- [ ] `impact.rs`：受影响模块、下游传播和 Closure 限制——**纯逻辑，不直接提交 Task**（§4.7）
- [ ] `submit.rs`：RebuildRestart / Resolve 任务提交
- [ ] `mod.rs`：RuntimeWatchEngine 和线程装配 + re-export
- [ ] `impact.rs` 独立单测（§4.7）：变更模块只扩散到允许的下游；外部依赖不当成本地源码模块传播；Closure 之外模块不被加入重建集合；同批事件收敛为一个任务
...
- [ ] §4.7 四条影响分析单测全部落地并通过
- [ ] 文件变更 → 增量构建/重启的端到端行为不变（既有 watch 测试全绿）
- [ ] 同一批事件仍收敛为一个任务（无重复提交）
- [ ] `impact.rs` 无任何 Task 提交依赖（代码走查确认）
- [ ] 四件套全绿；公共入口不变
...
- [ ] `classify.rs`（路径分类/ignore）
- [ ] `debounce.rs`（事件收集/去抖）
- [ ] `impact.rs`（纯影响分析 + 独立单测）
- [ ] `submit.rs`（任务提交）
- [ ] `mod.rs` 装配与 re-export
- [ ] 测试归位与四件套验证
