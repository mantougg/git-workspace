...
- [x] 内存缓存：`status_cache` 已缓存 status，LRU 上限生效（moka 并发 LRU，max_capacity 5000）
...
- [x] 单仓库状态刷新 < 100ms，普通增量刷新 < 300ms（release benchmark 实测 per-repo status 8.14ms）
- [x] 1000 仓库空闲内存 < 500MB（LRU 上限生效：max_capacity 5000，≈0.5MB）
- [x] 并发 status 时无 libgit2 跨线程句柄复用（缓存只放 `RepoStatus` 纯数据，无 libgit2 句柄）
- [x] 打开 Dashboard/刷新列表不触发任何网络 fetch（ahead/behind 仅本地 refs）
- [x] 变更 10 个文件只重算受影响的 N 个仓库（`find_affected_repos` 路径→仓库映射 + 测试）
...
| 2026-08-13 | ✅ | 完成剩余：status_cache 改 moka 并发 LRU（max_capacity 5000，≈0.5MB）；`cargo test` 27 passed；release benchmark 单仓库 status 8.14ms；与 T-06 联调随 T-06 进行

### 子任务清单

- [x] 设计 status 缓存 key 与失效策略（path key；LRU 上限已实现）
