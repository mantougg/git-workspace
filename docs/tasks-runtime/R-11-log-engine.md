...
| 2026-08-30 | ➕ 能力扩展（由 AI-03 驱动） | 引擎新增 `search_tail`（过滤+tail 语义：跨滚动段流式滑动窗口返回**最近** n 行匹配，活跃会话先读环形缓冲再过滤——`search` 只返回最早 n 行，不满足「最近错误日志」语义）；`RuntimeService` 补 `tail_logs` / `search_logs_tail` 查询包装（AI-03 Context Builder 的「日志尾部 / 最近错误日志」收集器使用）。不改既有 search/tail 行为，`cargo test --lib` 回归通过 |
