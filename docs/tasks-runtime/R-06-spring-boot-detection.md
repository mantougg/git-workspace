...
- [ ] 检测 `spring-boot-maven-plugin`（pom plugins，含 parent 继承场景）（§21）
- [ ] 检测 `@SpringBootApplication` 源码注解：轻量文本/正则扫描 `src/main/java`，**不建 Java AST / 代码索引**（全局约束 §1）
- [ ] 候选 Main Class 列表：全限定类名 + 所在模块（如 `com.example.Application / AdminApplication / GatewayApplication`）
- [ ] 默认 Main Class 自动推断（§22）：唯一候选直接选定；多候选按命名/插件配置启发式排序；pom `start-class` 属性优先
- [ ] 非 Spring Boot 模块（纯 Library）不产生候选
- [ ] 结果纳入 R-01 解析缓存一并失效/复用
...
- [ ] 样例工程候选 Main Class 列表完整，默认推断与 `mvn spring-boot:run` 实际启动类一致
- [ ] 多候选场景（Admin + Gateway）全部列出且可区分模块
- [ ] 纯 Library 模块不产生候选
- [ ] pom/源码变化后候选列表正确刷新
